...
async def notify(payload: NotifyIn, _=Depends(require_key)):
    ...
    if ch == "slack":
        if not SLACK_WEBHOOK: raise HTTPException(400,"slack not configured")
        body = {"text": f"*{payload.title or 'HIS/LIPE'}*\n{payload.text}"}
        await _post_json(SLACK_WEBHOOK, body); return {"ok": True}
    if ch == "discord":
        if not DISCORD_WEBHOOK: raise HTTPException(400,"discord not configured")
        body = {"content": f"**{payload.title or 'HIS/LIPE'}**\n{payload.text}"}
        await _post_json(DISCORD_WEBHOOK, body); return {"ok": True}
    raise HTTPException(400, f"unsupported channel: {payload.channel}")backend/app/integrations.py
