...
def simulate(
    ...
    if not s: raise HTTPException(404, "series_not_cached_call_timeseries_first")
    pts = json.loads(s)["points"]
    if len(pts) < 60: raise HTTPException(400, "insufficient_history")
    rets = _returns(pts)[-250:]  # last 250 steps
    if not rets: raise HTTPException(400, "no_valid_returns")
    ...
    p10  = sorted(paths)[int(0.10*len(paths))]
    p50  = sorted(paths)[int(0.50*len(paths))]
    p90  = sorted(paths)[int(0.90*len(paths))]
    return {
        "asof": pts[-1]["ts"],
        "last": last,
        "horizon": horizon,
