...
# Optional market lib (stocks/options)
try:
...
# Realistic-ish tile icons (HTTP images). Safe to replace with your own.
ICON_URLS = {
    "Lottery": "https://img.icons8.com/color/96/lotto.png",
    "Crypto": "https://img.icons8.com/color/96/bitcoin--v1.png",
    "Stocks": "https://img.icons8.com/color/96/combo-chart--v1.png",
    "Options": "https://img.icons8.com/color/96/options--v1.png",
    "Real Estate": "https://img.icons8.com/color/96/real-estate.png",
    "Commodities": "https://img.icons8.com/color/96/oil-industry.png",
    "Sports": "https://img.icons8.com/color/96/american-football.png",
    # diverse, mixed-ethnicity group icon for Human Behavior:
    "Human Behavior": "https://img.icons8.com/color/96/conference-foreground-selected.png",
    "Astrology": "https://img.icons8.com/color/96/crystal-ball.png",
}
...
.tile-img{ width:48px;height:48px;object-fit:contain;margin-bottom:8px; }
.tile-title{ font-weight:700;margin-bottom:4px;color:#fff;}
.tile-desc{ color:var(--muted);font-size:14px;min-height:36px;}
...
# >>> THIS IS THE LINE THAT WAS FAILING BEFORE — NOW FIXED AND CLOSED <<<
...
""", unsafe_allow_html=True)
    ...
    for row in rows:
        ...
        for col, (title, desc) in zip(cols, row):
            with col:
                if st.button(title, key=f"btn-{title}", use_container_width=True):
                    st.session_state.selected_module = title
                    st.rerun()
                icon = ICON_URLS.get(title, "")
                st.markdown(f"""
<div class="tile" onclick="document.querySelector('button[data-testid=&quot;btn-{title}&quot;]')?.click()">
  {'<img class="tile-img" src="'+icon+'" />' if icon else ''}
...
def page_crypto():
    ...
    if st.button("Refresh Prices", type="primary"):
        try:
            ...
            fig = px.bar(df, x="Coin", y="Price (USD)", title="Crypto Prices (USD)", template="plotly_dark")
        ...
        for t in [x.strip().upper() for x in tickers.split(",") if x.strip()]:
            try:
                ...
                fig = px.line(df, x="Date", y="Close",
                              title=f"{t} Close - {period} / {interval}",
...
def page_realestate():
    ...
    x = np.arange(60)
    y = 100 + np.cumsum(np.random.normal(0, 0.3, size=60))
    df = pd.DataFrame({"t": x, "Index": y})
    fig = px.line(df, x="t", y="Index", title="Market Tilt Demo", template="plotly_dark")
    st.plotly_chart(fig, use_container_width=True)

def page_commodities():
    st.markdown("### 🛢️ Commodities")
    x = list(range(60))
    y = [100 + (i*0.2) + random.uniform(-0.5,0.5) for i in x]
    df = pd.DataFrame({"t": x, "Price": y})
    fig = px.line(df, x="t", y="Price", title="Gold demo curve", template="plotly_dark")
    st.plotly_chart(fig, use_container_width=True)

def page_sports():
    st.markdown("### 🏈 Sports")
    st.info("Wire odds feeds (The Odds API / Sportradar) and LIPE overlays.")
    teams = ["Hawks","Sharks","Tigers","Giants"]
    odds = [random.randint(-150, 250) for _ in teams]
    df = pd.DataFrame({"Team":teams,"Moneyline":odds})
    fig = px.bar(df, x="Team", y="Moneyline", title="Moneylines (demo)", template="plotly_dark")
    st.plotly_chart(fig, use_container_width=True)
    if astro_on:
...
def page_behavior():
    ...
    if st.button("Run Sentiment Demo"):
        ...
        df = pd.DataFrame({"t": xs, "Sentiment": ys})
        fig = px.line(df, x="t", y="Sentiment",
                      title=f"Sentiment proxy — {kw}",
                      template="plotly_dark")
        st.plotly_chart(fig, use_container_width=True)
...
def page_astrology():
    ...
    df = pd.DataFrame(data)
    if not df.empty:
        fig = px.line(df, x="date", y="degree",
                      title=f"{planet} degrees over time",
                      template="plotly_dark")
        st.plotly_chart(fig, use_container_width=True)
