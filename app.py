...
def apply_theme_css():
    ...
    .stTabs [role="tablist"] button {{
      ...
      margin-right:.7rem; border-radius:10px; background:#111; color:{accent};
    }}
    .stTabs [role="tablist"] button[aria-selected="true"] {{
      background:{accent}; color:#000; transform:scale(1.05);
    }}
...
def entropy01_from_changes(*vals):
    arr=[v for v in vals if pd.notna(v)]
    if len(arr)<2: return .5
    s=np.std(arr); return float(1-min(max(s/20,0),1))

def safe_get(url,p=None,t=25):
    try:
        r=requests.get(url,params=p,headers=USER_AGENT,timeout=t)
...
def yf_multi(ticks,period="6mo"):
    ...
    for t in ticks:
        ...
        meta[t]={"market_cap":getattr(i,"market_cap",np.nan),
                 "last_price":getattr(i,"last_price",np.nan),
                 "volume":getattr(i,"last_volume",np.nan)}
