...
# Author credit: POWERED BY JESSE RAY LANDINGHAM JR

from __future__ import annotations
import math, time, io, os, json
...
# ---------- Optional providers (loaded lazily and safely) ----------
HAS_YF = False
try:
...
# ---------- Styles (Hero 3×, Pills 2×, Sticky header) ----------
CSS = """
<style>
:root{
...
.hero-wrap{position:sticky; top:0; z-index:9; padding:6px 0 12px 0; background:linear-gradient(180deg,rgba(11,18,32,.95),rgba(11,18,32,.80) 60%,rgba(11,18,32,0));}
.hero{
...
with st.sidebar:
    ...
    nav = st.radio("Go to", ["Dashboard", "Crypto", "Confluence", "US Market (All Listings)", "S&P 500", "Options", "Fusion", "Export", "Scores", "Settings"], index=0)
...
def safe_sigmoid(p: float|int|None) -> float:
    if p is None or (isinstance(p, float) and np.isnan(p)): return 0.5
    try:
        return 1.0/(1.0 + math.exp(-float(p)/10.0))
    except Exception:
        return 0.5

# ---------- Data: Crypto via CoinGecko ----------
@st.cache_data(ttl=90, show_spinner="Loading crypto from CoinGecko…")
def cg_markets(vs="usd", per_page=200) -> pd.DataFrame:
    url = "https://api.coingecko.com/api/v3/coins/markets"
    p   = {"vs_currency":vs,"order":"market_cap_desc","per_page":int(max(1,min(per_page,250))),"page":1,
...
def score(df: pd.DataFrame, wv=0.30, w24=0.25, w7=0.25, wl=0.20) -> pd.DataFrame:
    ...
    # RAW (crowd heat) & TRUTH (stability)
...
def get_stock_snapshot(tickers: List[str]) -> pd.DataFrame:
    """
    Multi-source priority: IEX → Finnhub → Alpha Vantage → yfinance.
    Returns columns: symbol, name, current_price (+ empty change cols).
    """
    tickers = [t.strip().upper() for t in tickers if t and t.strip()]
    if not tickers: return pd.DataFrame()

    # 0) IEX Cloud (requires paid/free key; faster, batched)
    IEX = _sec("IEX_API_KEY")
    if IEX:
        try:
            syms = ",".join(tickers[:1000])
            url  = f"https://cloud.iexapis.com/stable/stock/market/batch"
            params={"symbols":syms,"types":"quote","token":IEX}
    ...
    # 2) Alpha Vantage (5 calls/min free; light use)
    AV = _sec("ALPHAVANTAGE_API_KEY")
    if AV:
        try:
            rows=[]
            for t in tickers[:25]:  # rate limited
    ...
    # 3) yfinance (fallback; batches ok)
    if HAS_YF:
        try:
...
def pill_row(active:str) -> str:
    cols = st.columns(4)
    def pill(html_class, icon, text, key):
        is_active = (active==key)
        cla = f"pill {html_class}"+(" active" if is_active else "")
        clicked = st.button(f"{icon} {text}", key=f"pill_{key}")
        st.markdown(f'<div class="{cla}">{icon}&nbsp;{text}</div>', unsafe_allow_html=True)
        return clicked
    clicked = None
    with cols[0]:
        if st.button(" ", key="raw_space"): pass
        clicked = pill("raw","🔥","RAW","raw") or clicked
    with cols[1]:
        clicked = pill("truth","💧","TRUTH","truth") or clicked
    with cols[2]:
        clicked = pill("conf","⭐","CONFLUENCE","conf") or clicked
    with cols[3]:
        clicked = pill("delta","⚡","Δ (RAW↔TRUTH)","delta") or clicked
    # Use session_state to hold active metric
    if "metric" not in st.session_state: st.session_state.metric = active
    if clicked:
        # Find which button got pressed via the newly drawn order
        # Simple heuristic: toggle cycle
        order = ["raw","truth","conf","delta"]
        idx = (order.index(st.session_state.metric)+1)%4
        st.session_state.metric = order[idx]
...
def legend_strip():
    st.markdown(
        ...
        ⭐ CONFLUENCE = RAW + TRUTH agree, consistent & liquid &nbsp;&nbsp;|&nbsp;&nbsp;
        ⚡ Δ = |RAW − TRUTH| (gap)
        </div>
        """, unsafe_allow_html=True
    )

def chart_bar(df: pd.DataFrame, metric: str, topn:int=20, title:str=""):
    import plotly.express as px  # lazy import ensures app runs if missing earlier
    order = {"raw":"raw_heat", "truth":"truth_full", "conf":"confluence01", "delta":"delta01"}
    col = order.get(metric, "confluence01")
    d = df.sort_values(col, ascending=False).head(topn)
    fig = px.bar(d, x="symbol", y=col, hover_name="name", height=360)
...
def page_sp500():
    ...
    base = sp500_list()
    if base.empty:
        st.warning("Couldn’t fetch S&P list; try again later.")
        return
    ...
    if snap.empty:
        st.error("No stock data from any provider. Add a provider key in Secrets (recommended: IEX or Finnhub).")
        return
    merged = snap.merge(base, on="symbol", how="left")
    df = score(merged, w_vol, w_m24, w_m7, w_liq)
    kpis(df, "S&P 500")
    chart_bar(df, "conf", topn=20)
...
def page_us_all():
    ...
    uni = us_listings()
    if uni.empty:
        st.warning("US listings unavailable right now.")
        return
    ...
    if snap.empty:
        st.error("No stock data from providers. Add IEX/FINNHUB/ALPHAVANTAGE secret."); return
...
def page_options():
    hero()
    if not HAS_YF:
        st.error("Options require yfinance. Add `yfinance` to requirements and reboot.")
        return
    base = us_listings()
    sym = st.text_input("Ticker (e.g., AAPL)", value="AAPL").strip().upper() if base.empty else \
    ...
    with c1:
        st.caption("Calls (sorted by OI/Volume)")
        if calls.empty: st.info("No calls data.")
        else: table(calls.sort_values(["openInterest","volume"], ascending=False).head(25), [c for c in keep if c in calls.columns], "Calls")
    with c2:
        st.caption("Puts (sorted by OI/Volume)")
        if puts.empty: st.info("No puts data.")
        else: table(puts.sort_values(["openInterest","volume"], ascending=False).head(25), [c for c in keep if c in puts.columns], "Puts")
...
def page_fusion():
    ...
    uni = us_listings()
    if not uni.empty:
        snap = get_stock_snapshot(uni[uni["ETF"]!="Y"]["symbol"].tolist()[:120]) if "ETF" in uni.columns else get_stock_snapshot(uni["symbol"].tolist()[:120])
...
def page_export():
    hero()
    dfc = score(cg_markets(per_page=200), w_vol, w_m24, w_m7, w_liq)
    st.download_button("Download Crypto CSV", data=dfc.to_csv(index=False).encode("utf-8"), file_name="crypto_truth_raw_conf.csv", mime="text/csv")
    uni = us_listings()
    if not uni.empty:
        snap = get_stock_snapshot(uni["symbol"].tolist()[:400])
        if not snap.empty:
            dfs = score(snap.merge(uni, on="symbol", how="left"), w_vol, w_m24, w_m7, w_liq)
            st.download_button("Download US Snapshot CSV", data=dfs.to_csv(index=False).encode("utf-8"), file_name="us_truth_raw_conf.csv", mime="text/csv")
...
def page_settings():
    hero()
    st.caption("Personalization & saved presets coming soon.")
