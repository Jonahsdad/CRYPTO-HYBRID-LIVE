...
# ====================== CONFIG ======================
APP_NAME = "Crypto Hybrid Live — Phase 5 (Portfolios + Share + Mini Charts)"
st.set_page_config(page_title=APP_NAME, layout="wide")
USER_AGENT = {"User-Agent":"Mozilla/5.0 (CHL-Phase5)"}

FEATURES = {
    "REDDIT": True,
    "DEFI_LLAMA": True,
    "CRYPTOPANIC": True,   # uses st.secrets["CRYPTOPANIC_KEY"] if present
    "ALERTS": True,
    "SNAPSHOT": True,
    "SPARKLINES": False,   # per-row sparklines later (kept off for perf)
}
CG_PER_PAGE = 150
HIST_DAYS = 90           # for backtests & mini charts

# ====================== THEME (slightly bigger UI) ======================
...
def apply_theme_css():
    ...
    TAB_FONT, TABLE_FONT, HDR_FONT = "1.35rem","1.20rem","1.9rem"
    KPI_VAL, KPI_LBL = "2.4rem","1.15rem"
    st.markdown(f"""
    <style>
    html, body, [class*="css"] {{
      font-size: 18px; line-height: 1.6; background:{base_bg}; color:{base_fg};
    }}
    .stTabs [role="tablist"] button {{
      font-size:{TAB_FONT} !important; font-weight:700 !important; padding:0.8rem 1.6rem !important;
      border-radius:10px !important; margin-right:0.8rem !important; background:#111 !important; color:{accent} !important; border:1px solid #222 !important;
    }}
    .stTabs [role="tablist"] button[aria-selected="true"] {{
      background:{accent} !important; color:#000 !important; transform:scale(1.04);
    }}
...
def lipe_truth(df, w):
    ...
    truth=( w["w_vol"]*(df["vol_to_mc"]/2).clip(0,1).fillna(0.0)
...
def ensure_profile():
    if "profile" not in st.session_state:
        st.session_state["profile"]={"watchlist":[], "perf_mode":False, "saved_presets":{}}
...
def fetch_markets(vs="usd", per_page=CG_PER_PAGE, spark=False):
    ...
    p={"vs_currency":vs,"order":"market_cap_desc","per_page":per_page,"page":1,
...
def get_market_chart(coin_id:str, vs="usd", days=HIST_DAYS):
    """Return DataFrame with columns: ts, price."""
    url=f"https://api.coingecko.com/api/v3/coins/{coin_id}/market_chart"
    r=safe_get(url, params={"vs_currency":vs,"days":days}, timeout=30)
    if not r: return pd.DataFrame([])
    try:
...
# ====================== HEADER ======================
st.title("🟢 "+APP_NAME)
st.caption("Truth > Noise • Phase 5 adds **Portfolios/Backtests**, **Shareable Links**, **Mini Charts**, **Top Picks**")
...
with st.sidebar:
    ...
    st.session_state["theme"]="dark" if theme_pick=="Dark" else "light"; apply_theme_css()
